...
Migrations and Seeds are tracked in your database. Each time the DB is opened, the `SequelizeMeta` table will be checked and only new migrations and seeds will be run.
...
- Single package.json setup, now part of ERB v17.x.x
...
- `config/`
  - `webpack.config.base`, `config/webpack.config.renderer.xxx` : using `CleanWebpackPlugin` and `CopyWebpackPlugin` copies the `models`, `model-seeds` `migrations` folders to `app/dist` on build creates two renderer bundles. See `package.json.copyToDistFolders` for folder array used to manage these plugins.
...
- `models/`_new_: Sequelize 4.x models. These work just like the Sequelize docs.
- `test/db/` _new_: Test for database.
- `package.json` _updated_: New script, db-migrate, for running migrations as well as some additional packages. New section added for folder management called `copyToDistFolders`. Each folder will be cleaned and filled when running `yarn run package` so the contents is fresh.

> I broke Sequelize convention by calling the seeder directory `model-seeds` because I wanted to keep it near the `models` and `migrations` directories.
